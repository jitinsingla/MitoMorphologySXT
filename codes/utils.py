...
ContourDF = '/home/anshu/Documents/Github_push/Output/Contours_analysis/'
def RadialContour(jsonFileList, conditions, size = "global", df_save = False, save_edt = False):
    ...
    for i in range(len(jsonFileList)):
        ...
        # If user want to visualize and save the slice and data
        ...
        # plt.plot(enrichment)
        # plt.title(f"Enrichment plot of {conditions[cell.data['name']]['cell']}")
        ...
        # plt.title(f"Observed Mito Distribution of {conditions[cell.data['name']]['cell']}")
...
def line_plot(
    ...
    if show_yticks:
        ax.tick_params(axis='y', labelsize=4, length=1.5, width=0.4)
    else:
        ax.set_yticklabels([])
    ...
    # ==================================================
    if show_xticks:
        ax.tick_params(axis='x', labelsize=4, length=1.5, width=0.4)
    else:
        ax.set_xticklabels([])
...
def pointplot(
    ...
    ax.set_xticks(x_vals[::2])      # every second value
    ax.set_xticklabels(x_vals[::2])
